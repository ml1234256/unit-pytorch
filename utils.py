...
#     def __init__(self, root, size_w=360, transform=None):
#         img_list = os.listdir(root)
#         self.img_paths = [os.path.join(root, k) for k in img_list]
#         self.img_name = img_list
#         self.transform = transform
...
#         if self.transform:
#             img_data = self.transform(img)
#             img_a = img_data[:, :, :self.size_w]
#             img_b = img_data[:, :, self.size_w:]
#
#         else:
#             img_data = torch.from_numpy(np.array(img))
#             img_a = img_data[:, :self.size_w, :]
#             img_b = img_data[:, self.size_w:, :]
#
#         return {'A': img_a, 'B': img_b, 'img_name': img_name}
#
#     def __len__(self):
#
#         return len(self.img_paths)
...
class ReadConcat(Dataset):
    ...
    def __getitem__(self, index):
        ...
        w_offset = random.randint(0, max(0,
                                         w - self.opt.fineSize - 1))  # random.randint(a,b), generate an int between a and b
